#!/usr/bin/env node
// gellmann — Claude Code SubagentStart hook
//
// SessionStart context is parent-thread only and never reaches subagents, so
// without this every Task-spawned agent runs gellmann-unaware (issue #252).
// When gellmann mode is active, inject the same ruleset into each subagent.
//
// Scoping (opt-in, issue #506): set GELLMANN_SUBAGENT_MATCHER to a regex and
...
const { readMode, writeHookOutput } = require('./gellmann-runtime');

const mode = readMode();

// Absent flag or off → gellmann isn't active; inject nothing.
if (!mode || mode === 'off') {
  process.exit(0);
}
...
function inject() {
  try {
    writeHookOutput('SubagentStart', mode, getGellmannInstructions(mode));
  } catch (e) {
...
let matcherRe = null;
try {
  if (process.env.GELLMANN_SUBAGENT_MATCHER) {
    matcherRe = new RegExp(process.env.GELLMANN_SUBAGENT_MATCHER, 'i');
  }
} catch (e) {
  matcherRe = null;
}
