#!/usr/bin/env node
// gellmann — Claude Code SessionStart activation hook
//
// Runs on every session start:
//   1. Writes flag file at $CLAUDE_CONFIG_DIR/.gellmann-active (defaults to ~/.claude; statusline reads this)
//   2. Emits gellmann ruleset as hidden SessionStart context
//   3. Detects missing statusline config and emits setup nudge

const fs = require('fs');
const path = require('path');
const { getDefaultMode, getClaudeDir, isShellSafe } = require('./gellmann-config');
...
// 2. Emit the gellmann ruleset, filtered to the active mode.
let output = getGellmannInstructions(mode);
...
if (!isCodex && !isCopilot) try {
  ...
  if (!hasStatusline && !fs.existsSync(nudgeFlagPath)) {
    ...
    const scriptName = isWindows ? 'gellmann-statusline.ps1' : 'gellmann-statusline.sh';
    ...
    if (isShellSafe(scriptPath)) {
      ...
      output += "\n\n" +
        "STATUSLINE SETUP NEEDED: The gellmann plugin includes a statusline badge showing active mode " +
        "(e.g. [GELLMANN:WORK], [GELLMANN:SOLO]). It is not configured yet. " +
        "To enable, add this to " + settingsPath + ": " +
        statusLineSnippet + " " +
        "Proactively offer to set this up for the user on first interaction.";
    } else {
      // gellmann: install path has shell metacharacters — don't embed it in a
