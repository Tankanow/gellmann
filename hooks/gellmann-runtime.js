...
const os = require('os');
const { getClaudeDir, getConfigDir } = require('./gellmann-config');

const STATE_FILE = '.gellmann-active';

// gellmann: VS Code Copilot never sets COPILOT_PLUGIN_DATA — it only injects
// CLAUDE_PLUGIN_ROOT, pointed at an install path under .vscode/agent-plugins/
// (#528). Without this fallback isCopilot was false, so gellmann assumed
...
// Live mode written by activate/mode-tracker. Absent flag = gellmann off.
function readMode() {
  try {
...
function writeHookOutput(event, mode, context = '') {
  ...
  if (isCodex) {
    const output = { systemMessage: `GELLMANN:${mode.toUpperCase()}` };
    if (context) {
