...
name: gellmann-solo
description: >
  Solo persona of the Gell-Mann lens: no colleague to ask, so find the people
  who wrote the field down. Locates the canonical primary source for a domain
  and the human behind it: the specification or RFC and its editors, the
  official reference for the exact version in use, the upstream source and
  its maintainer, the paper and its authors, the standard and its committee.
  Returns a short briefing on the domain plus openable citations and the
  named humans worth reading. Secondary sources count only as published
  critiques of a named primary. Use when the user says "source?", "cite
  that", "who would know this", "what does the spec say", "check the docs",
  or invokes /gellmann-solo, or when solo mode is active and a claim rests on
  knowledge the agent is reconstructing from memory. Switches the session to
  solo mode.
