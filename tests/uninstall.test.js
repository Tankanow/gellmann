...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'gellmann-uninstall-'));
...
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash /some/path/gellmann-statusline.sh' },
}));
...
assert.equal(
  ...
  undefined,
  'gellmann statusLine entry must be removed',
);
...
// #374: a combined statusline (another plugin && gellmann) must keep the other
// plugin's part — uninstall must not nuke the whole command or leave a husk.
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash ~/caveman-statusline.sh && bash /p/gellmann-statusline.sh' },
}));
...
assert.equal(
  ...
  'a combined statusLine must keep the non-gellmann command',
);
...
const malformedSettings = '{ "statusLine": { "command": "gellmann-statusline.sh", broken';
