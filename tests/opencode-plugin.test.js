...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gellmann-opencode-'));
process.env.XDG_CONFIG_HOME = tmp;
// Force a known default — without this gellmann would auto-detect against
// this repo's own git history and the default-mode assertion below would be
// unstable.
process.env.GELLMANN_DEFAULT_MODE = 'solo';
const statePath = path.join(tmp, 'opencode', '.gellmann-active');

let loadPlugin, parseCommandFile;
test.before(async () => {
  const url = pathToFileURL(path.join(__dirname, '..', '.opencode', 'plugins', 'gellmann.mjs'));
  ...
  parseCommandFile = require(path.join(__dirname, '..', '.opencode', 'plugins', 'gellmann-frontmatter.cjs')).parseCommandFile;
});
...
test('system.transform injects the ruleset at the default mode (solo)', async () => {
...
test('command.execute.before persists /gellmann work, transform follows it', async () => {
  const hooks = await loadPlugin({});
  await hooks['command.execute.before']({ command: 'gellmann', arguments: 'work', sessionID: 's' });
  assert.equal(fs.readFileSync(statePath, 'utf8'), 'work');
  const system = await transform(hooks);
  assert.match(system[0], /GELLMANN MODE ACTIVE — mode: work/);
});

test('/gellmann off persists off and transform injects nothing', async () => {
  const hooks = await loadPlugin({});
  await hooks['command.execute.before']({ command: 'gellmann', arguments: 'off', sessionID: 's' });
...
test('system.transform merges into existing system entry (Qwen compat, #296)', async () => {
  ...
  assert.match(output.system[0], /GELLMANN MODE ACTIVE/);
});

test('unsupported /gellmann arguments do not reset the current mode', async () => {
  const hooks = await loadPlugin({});
  fs.writeFileSync(statePath, 'work');
  await hooks['command.execute.before']({ command: 'gellmann', arguments: 'status', sessionID: 's' });
  assert.equal(fs.readFileSync(statePath, 'utf8'), 'work');
});
