...
// always-on context, commands/*.toml for /gellmann + /gellmann-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring gellmann.
...
const REUSED_COMMANDS = ['commands/gellmann.toml', 'commands/gellmann-review.toml'];
const REUSED_SKILLS = ['skills/gellmann/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. Gellmann's
...
test('manifest names the gellmann extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the gellmann rules', () => {
  const manifest = loadManifest();
